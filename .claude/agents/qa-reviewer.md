...
- 资源泄漏：文件/连接/客户端等资源须有明确关闭机制（context manager 或 try/finally）
- 错误处理：禁止静默吞掉异常（裸 `except: pass` 或 `except Exception: pass`）
- 并发安全：共享状态须有适当保护（threading.Lock 等）
- 内部错误泄漏：异常信息不得包含敏感数据（API Key、内部路径、完整 URL）
- 硬编码密钥：禁止任何形式的硬编码凭证
- **smoke_test.sh 覆盖缺口：本次需求新增或修改的对外接口，若 `scripts/smoke_test.sh` 未补充对应断言，视为 Critical**
