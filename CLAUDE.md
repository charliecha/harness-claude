# harness-claude — AI 工程准则 (Layer 1: Rule)

## 强制规则（红线，不可违反）

本项目红线分两层：
- **通用红线**见 [.harness/rules/common.md](.harness/rules/common.md)
- **语言专属红线**见 `.harness/rules/$LANG.md`（$LANG 来自 [.harness-config.json](.harness-config.json) 的 `language` 字段，当前为 `python` → `.harness/rules/python.md`）
